angular.module('sessionControllers', [])
    ...
    return $resource('/Purchase/sessions/:id/',{id:'@id'},{
        update: {method:'PUT'},
    },{
...
function($scope,$stateParams, Session) {
    $scope.session = Session.get({id:$stateParams.id}); //Get a single session.Issues a GET to /api/sessions/:id
  }
)
...
function($scope, $state, $stateParams, Session) {
  ...
  $scope.session = Session.get({id:$stateParams.id});
  };
